...
def get_args():
    ...
    parser.add_argument("--left_margin", type=float, default=0.08,
                        help="extra space on the left to the word boundary")
    parser.add_argument("--right_margin", type=float, default=0.08,
                        help="extra space on the right to the word boundary")
...
if __name__ == "__main__":
    ...
    for item in manifest:
        ...
        smaller_indx = []
        alignment_fn = os.path.join(
            args.audio_root, "aligned", item[0].replace(".wav", ".csv"))
        ...
        for ind_inter, editType in zip(all_ind_intervals, editTypes):
            ...
            mi = (max(mi[0] - args.left_margin, 1/args.codec_sr),
                  min(mi[1] + args.right_margin, audio_dur))  # in seconds
            mis.append(mi)
    ...
    for i, (audio_fn, target_text, mask_interval) in enumerate(tqdm.tqdm(zip(audio_fns, target_texts, mask_intervals))):
        ...
        mask_interval = [
            [round(cmi[0]*args.codec_sr), round(cmi[1]*args.codec_sr)] for cmi in mask_interval]
        ...
        orig_audio, new_audio = inference_one_sample(
            model, model_args, phn2num, text_tokenizer, audio_tokenizer, audio_fn, target_text, mask_interval, args.device, vars(args))
        ...
        if not os.path.isfile(save_fn_orig):
            ...
            if orig_sr != args.codec_audio_sr:
                orig_audio = torchaudio.transforms.Resample(
                    orig_sr, args.codec_audio_sr)(orig_audio)
