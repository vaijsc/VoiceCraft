...
def topk_sampling(logits, top_k=10, top_p=1.0, temperature=1.0):
    ...
    # top_k: (`optional`) int
    #     The number of highest probability vocabulary tokens to keep for top-k-filtering. Between 1 and infinity. Default to 10.
...
class VoiceCraft(
    ...
    def load_pretrained_model(self, sd, new_vocab_size):
        # Handle the text embedding part
        old_embedding_weights = sd['text_embedding.word_embeddings.weight']
        old_vocab_size, embedding_dim = old_embedding_weights.size()
            
        # Create new embedding weights for the new phonemes
        new_embedding_weights = torch.randn(new_vocab_size - old_vocab_size, embedding_dim)
            
        # Combine old and new embedding weights
        combined_embedding_weights = torch.cat((old_embedding_weights, new_embedding_weights), dim=0)
        assert combined_embedding_weights.size() == (new_vocab_size, embedding_dim)
            
        # Update the text embedding weights in the state dictionary
        sd['text_embedding.word_embeddings.weight'] = combined_embedding_weights
