...
# all the models are cached and uploaded to replicate.delivery for faster booting
MODEL_URL = "https://weights.replicate.delivery/default/pyp1/VoiceCraft-models.tar"
...
class Predictor(BasePredictor):
    def setup(self):
        ...
        for voicecraft_name in [
            ...
            self.ckpt[voicecraft_name] = torch.load(
                ckpt_fn, map_location="cpu")
        ...
        self.text_tokenizer = EnTextTokenizer(backend="espeak")
        self.audio_tokenizer = AudioTokenizer(
            signature=encodec_fn, device=self.device)

        align_model = WhisperxAlignModel()
        self.transcribe_models = {
            k: WhisperxModel(
                f"{MODEL_CACHE}/whisperx_{k.split('.')[0]}", align_model)
    ...
    def predict(
        ...
        voicecraft_model: str = Input(
            ...
            choices=["giga830M.pth", "giga330M.pth",
        ...
        if task == "zero-shot text-to-speech":
            ...
            orig_transcript_until_cutoff_time = " ".join(
                [word_bound["word"]
                    for word_bound in state["word_bounds"][: idx + 1]]
            )
        else:
