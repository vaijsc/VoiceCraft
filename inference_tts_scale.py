...
def get_args():
    ...
    parser.add_argument("--manifest_fn", type=str,
                        default="path/to/eval_metadata_file")
...
@torch.no_grad()
def inference_one_sample(model, model_args, phn2num, phn_seq, audio_tokenizer, audio_fn, target_text, device, decode_config, prompt_end_frame):
    # phonemize
    ...
    text_tokens = [
        phn2num[phn] for phn in phn_seq if phn in phn2num
    ]
    ...
    assert original_audio.ndim == 3 and original_audio.shape[
        0] == 1 and original_audio.shape[2] == model_args.n_codebooks, original_audio.shape
    logging.info(
    ...
    if decode_config['sample_batch_size'] <= 1:
        ...
        concat_frames, gen_frames = model.inference_tts(
            ...
            silence_tokens=eval(decode_config['silence_tokens']) if type(
                decode_config['silence_tokens']) == str else decode_config['silence_tokens']
        )  # output is [1,K,T]
    else:
        logging.info(
...
if __name__ == "__main__":
    ...
    manifest = manifest[1:]
    manifest = [[item[0], item[2], item[3], item[1], item[5]]
    ...
    for i, (audio_fn, text, prompt_end_frame, new_audio_fn, to_syn) in enumerate(tqdm.tqdm((zip(audio_fns, texts, prompt_end_frames, new_audio_fns, text_to_syn)))):
        ...
        concated_audio, gen_audio = inference_one_sample(
            model, model_args, phn2num, text_tokenizer, audio_tokenizer, audio_fn, text, args.device, vars(args), prompt_end_frame)

        # save segments for comparison
        concated_audio, gen_audio = concated_audio[0].cpu(), gen_audio[0].cpu()
        if output_expected_sr != args.codec_audio_sr:
            gen_audio = torchaudio.transforms.Resample(
                output_expected_sr, args.codec_audio_sr)(gen_audio)
            concated_audio = torchaudio.transforms.Resample(
                output_expected_sr, args.codec_audio_sr)(concated_audio)
        ...
        torchaudio.save(seg_save_fn_concat, concated_audio,
                        args.codec_audio_sr)
