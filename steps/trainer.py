...
class Trainer:
    ...
    def _setup_meters(self):
        """
        Set up performance meters for tracking various metrics during training and validation.

        This function creates a dictionary of AverageMeter objects for different metrics,
        including loss, accuracy, and timing measurements for both training and validation phases.

        Returns:
            dict: A dictionary containing AverageMeter objects for each metric.
        """
        meters = {}
    ...
    def _setup_models(self):
        ...
        if self.args.load_model_from != None and self.progress['step'] <= 1:
            ...
            print('loading model...')
            print(f"pretrained text vocab size: {sd['text_embedding.word_embeddings.weight'].shape[0]}")
            print(f'current text vocab size: {self.args.text_vocab_size + 1}')

            # if the model use the same vocab as the checkpoint 
            if self.args.text_vocab_size + 1 == sd['text_embedding.word_embeddings.weight'].shape[0]:
                model.load_state_dict(sd)
            # if the model use different vocab size
            else:
                model.load_pretrained_model(sd, self.args.text_vocab_size + 1)
            del sd
